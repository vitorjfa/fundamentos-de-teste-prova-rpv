...
function calcularPlano(plano: string, personal: boolean): IResultadoPlano {
  ...
  const planosBase: Record<string, number> = {
    mensal: 99.9,
    trimestral: 249.9,
    anual: 899.9,
  };

  const mesesPorPlano: Record<string, number> = {
    mensal: 1,
    trimestral: 3,
    anual: 12,
  };

  if (!Object.prototype.hasOwnProperty.call(planosBase, plano)) {
    return { valorMensal: 0, valorTotal: 0, ehValido: false };
  }

  const baseTotal = planosBase[plano];
  const meses = mesesPorPlano[plano];
  const adicionalPersonalTotal = personal ? 50 * meses : 0;

  const valorTotal = parseFloat(
    (baseTotal + adicionalPersonalTotal).toFixed(2),
  );
  const valorMensal = parseFloat(
    (baseTotal / meses + (personal ? 50 : 0)).toFixed(2),
  );
...
function realizarCheckin(checkin: ICheckin): IResultadoCheckin {
  ...
  if (aluno.vencimento.getTime() < checkin.horario.getTime()) {
  ...
  const dataStr = checkin.horario.toISOString().slice(0, 10);
  const jaFezHoje = checkIns.some(
    (r) => r.alunoId === checkin.alunoId && r.data === dataStr,
  );
  if (jaFezHoje) {
    return { permitido: false, mensagem: "Check-in já realizado no mesmo dia" };
  }

  checkIns.push({ alunoId: checkin.alunoId, data: dataStr });
  return { permitido: true, mensagem: "Check-in realizado" };
}
...
function cancelarPlano(alunoId: number): IResultadoCancelamento {
  ...
  if (!aluno) return { multa: 0, ehValido: false };
  if (!aluno.ativo) return { multa: 0, ehValido: false };

  const hoje = new Date();
  const diffMs = aluno.vencimento.getTime() - hoje.getTime();
  const diffDays = Math.ceil(diffMs / (1000 * 60 * 60 * 24));
  if (diffDays <= 0) return { multa: 0, ehValido: false };

  const mesesRestantes = Math.ceil(diffDays / 30);
  const resultadoPlano = calcularPlano(aluno.plano, aluno.personal);
  const valorMensal = resultadoPlano.valorMensal;
  const valorRestante = parseFloat((mesesRestantes * valorMensal).toFixed(2));
  const multa = parseFloat((valorRestante * 0.2).toFixed(2));
...
validar({
  descricao: "calcularPlano() - Plano mensal sem personal",
  atual: teste1.valorMensal,
  esperado: 99.9,
});
...
validar({
  descricao: "calcularPlano() - Plano anual sem personal",
...
validar({
  descricao: "realizarCheckin() - Check-in válido",
  atual: teste3.permitido,
  esperado: true,
});
...
validar({
  descricao: "realizarCheckin() - Check-in duplicado no dia",
  atual: teste4.permitido,
  esperado: false,
});
...
validar({
  descricao: "realizarCheckin() - Check-in fora do horário (5h)",
  atual: teste5.permitido,
  esperado: false,
});
...
validar({
  descricao: "cancelarPlano() - Cancelamento com multa 20%",
...
validar({
  descricao: "realizarCheckin() - Aluno inadimplente bloqueado",
  atual: teste7.permitido,
  esperado: false,
});
...
validar({
  descricao: "calcularPlano() - Plano mensal com personal",
  atual: teste8.valorMensal,
  esperado: 149.9,
});
...
validar({
  descricao: "calcularPlano() - Plano anual com personal total",
...
validar({
  descricao: "calcularPlano() - Plano inválido",
  atual: teste10.ehValido,
  esperado: false,
});
