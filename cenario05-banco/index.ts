...
function transferir(dados: ITransferir): boolean {
  ...
  const contaOrigem = contas.find((c) => c.id === dados.contaOrigemId);
  const contaDestino = contas.find((c) => c.id === dados.contaDestinoId);
  ...
  if (dados.valor > 5000.0) {
    return false;
  }

  const taxa = contaOrigem.banco !== contaDestino.banco ? 2.5 : 0;
  const valorTotal = dados.valor + taxa;

  if (valorTotal > contaOrigem.saldo) {
    return false;
  }

  contaOrigem.saldo = contaOrigem.saldo - valorTotal;
  contaDestino.saldo = contaDestino.saldo + dados.valor;
...
validar({
  descricao: "depositar() - Depósito válido R$100",
  atual: teste1,
  esperado: true,
});
...
validar({
  descricao: "depositar() - Depósito abaixo do mínimo",
  atual: teste2,
  esperado: false,
});
...
validar({
  descricao: "sacar() - Saque válido R$200",
  atual: teste3,
  esperado: true,
});
...
validar({
  descricao: "sacar() - Saque acima do saldo",
  atual: teste4,
  esperado: false,
});
...
validar({
  descricao: "transferir() - Mesmo banco sem taxa",
  atual: teste5,
  esperado: true,
});
...
validar({
  descricao: "transferir() - Outro banco com taxa R$2,50",
  atual: contaFernanda.saldo,
  esperado: 797.5,
});
...
validar({
  descricao: "transferir() - Acima do limite R$5000",
  atual: teste7,
  esperado: false,
});
...
validar({
  descricao: "depositar() - Conta inativa",
  atual: teste8,
  esperado: false,
});
...
validar({
  descricao: "Saldo correto após múltiplas operações",
  atual: contaAposTeste9.saldo,
  esperado: 1000,
});
...
validar({
  ...
  atual: contaTeste10.extrato.length,
  esperado: 1,
});
